...
API 无状态，服务端不维护会话状态，Token 采用 JWT 格式。鉴权按「本地 / 云端」双层划分（详见 [4.5.1 设备绑定简化认证模型](#451-设备绑定简化认证模型v1)）：

- **本地操作**（排盘、档案读写、本地规则分析）：由设备密钥（`device_key`）识别身份，**无需任何云端 Token**，离线完全可用。
- **云端操作**（AI 深度分析 LLM 调用、真太阳时校准等需云端算力的端点）：使用 JWT，由 `Authorization: Bearer <token>` 携带。
...
| 鉴权 | 本地端点（排盘/档案/本地分析）由 `device_key` 识别、无需 Token；云端端点需 `Authorization: Bearer <jwt>`（详见 §4.5.1） |
...
> **版本存储实现约束（ARC-15-P1-003 已确认）：** V1 采用**全快照（Snapshots Only）**方案——每次修改存完整档案 JSON 快照（`version_history.snapshot` 为 TEXT/JSON 类型），版本间 diff 由查询时动态计算，不在存储层持久化 diff。GET 上述端点即返回对应完整快照。
...
// Request
{
  ...
  "nickname": "string, optional, 1-32 chars, 不提供则默认用 username",
  "timezone": "string, optional, IANA tz 标识（如 'Asia/Shanghai'），注册时未提供则按 birth_place 反查默认时区"
}
...
// Response 201 Created
{
  ...
  "data": {
    ...
    "pillars": {
      "year": { "heavenly_stem": "庚", "earthly_branch": "午", "hidden_stems": ["丁", "己"], "na_yin": "路旁土" },
      "month": { "heavenly_stem": "甲", "earthly_branch": "申", "hidden_stems": ["庚", "壬", "戊"], "na_yin": "井泉水" },
      "day": { "heavenly_stem": "戊", "earthly_branch": "寅", "hidden_stems": ["甲", "丙", "戊"], "na_yin": "城头土" },
      "hour": { "heavenly_stem": "戊", "earthly_branch": "午", "hidden_stems": ["丁", "己"], "na_yin": "路旁土" }
    },
    "five_elements": {
    ...
    "yong_shen": { "element": "金", "method": "扶抑法", "explanation": "日主戊土偏旺，取金泄土生水为用", "confidence": 0.85, "data_tier": "inference", "alternative": [ { "element": "水", "method": "调候法", "confidence": 0.65, "data_tier": "inference" } ] },
    "xi_shen": { "element": "水", "relation_to_yong": "金生水，喜神生用神", "confidence": 0.75, "data_tier": "inference" },
    "ji_shen": { "element": "火", "reason": "火生土，加剧日主偏旺之势", "confidence": 0.80, "data_tier": "inference" },
    "pattern": "杀印相生格",
...
> **补充字段定义（落地自 ARC-15-P2-001）：**
> - `pillars.*.na_yin` — 该柱干支的纳音五行（如 `"路旁土"`）。
> - `yong_shen` / `xi_shen` / `ji_shen` — 改为**结构化对象**：`element`（五行）、`method`（推导方法，如扶抑法/调候法/通关法/病药法）、`explanation` / `reason` / `relation_to_yong`、`confidence`（0.0-1.0）、`data_tier`（`fact`/`inference`/`pending`）；`yong_shen` 另含 `alternative` 备选用神列表。具体字段与枚举值、置信度计算方式**【待产品负责人确认具体结构】**。
> - `pattern` — 命局格局名称（如 `"杀印相生格"`）；`pattern_category` — 格局大类枚举：`normal`（正格）/ `special`（特殊格）/ `none`（无格局）。格局枚举完整列表**【待产品负责人确认】**。
> - `shen_sha` — 神煞列表，key 为神煞名称、value 为出现的柱数组（如 `{ "天乙贵人": ["hour"] }`）；仅收录被广泛认可的约 15-20 种神煞，具体列表**【待产品负责人确认】**。
...
> **补充字段定义（落地自 ARC-15-P2-001）：**
> - `sections[].metadata` 完整结构：`fact_count` / `inference_count` / `pending_count`（事实/推理/推测计数）、`source`（内容生成来源，如 `bazi_engine.v1` / `ai.deepseek-chat-v3` / `user_input` / `manual`）、`related_sections`（关联 section_id 列表）、`key_terms`（关键命理术语）、`generated_at`（ISO 8601）、`ai_model`（生成模型标识）、`review_status`（`unreviewed` / `confirmed` / `disputed`）。每个 section 均应携带 metadata。
> - `confidence` 置信度体系：字符串枚举 `high` / `medium` / `low` 建议映射数值——`high`（0.80-1.00，确定性算法输出或多源一致确认）、`medium`（0.50-0.79，单一学派推理或 AI 单次分析）、`low`（0.00-0.49，推测性建议、多学派分歧）。排盘层固定为 `high`（确定性算法），AI 推测层默认 ≤ 0.40。**【待产品负责人确认具体数值计算方式与是否允许客户端按 confidence 过滤】**。
> - `data_tier` 直接派生自 ADR-012（数据三层体系）：`fact` → 事实性、`inference` → 分析性、`pending` → 推测性。
...
> **补充字段定义（落地自 ARC-15-P2-001）：**
>
> **术语详情（`GET /api/v1/knowledge/terms/{term_id}` 响应）：** 在列表字段基础上扩展为——`definition`（完整定义文本，支持 Markdown）、`aliases`（别名列表）、`related_terms`（带关系类型的对象数组，如 `{ "term_id": "t_002", "name": "正官", "relationship": "子类别" }`）、`sources`（经典典籍引用来源数组）、`examples`（应用示例数组）、`visibility`（`public` / `premium`）、`data_tier`（ADR-012 三层标记）。
>
> **引用来源结构（Source 对象，统一用于 `sources` 等位置）：**
> | 字段 | 类型 | 必填 | 描述 |
> |------|------|------|------|
> | `source_id` | string | ✅ | 来源唯一标识 |
> | `name` | string | ✅ | 典籍/文章名称（如 `"渊海子平"`）|
> | `author` | string | ❌ | 作者 |
> | `era` | string | ❌ | 成书年代 |
> | `chapter` | string | ❌ | 章节引用 |
> | `citation` | string | ❌ | 原文引用文本 |
> | `reliability` | string | ❌ | 可靠度：`canonical`(经典) / `reference`(参考) / `modern`(现代) |
> | `data_tier` | string | ✅ | ADR-012 三层标记 |
>
> **知识图谱关系类型（建议枚举，待产品负责人确认完整列表）：** `hierarchy.parent` / `hierarchy.child`（父子类别）、`synonym`（同义）、`antonym`（对立）、`derivation`（派生）、`reference`（引用）、`conflict`（学派分歧）、`see_also`（参见）。`related_terms` 中通过 `relationship` 字段引用上述类型。**【待产品负责人确认：图谱存储方式（SQLite 关系表 / JSON 内嵌 / 轻量图数据库）、维护机制（AI 自动构建 vs 专家录入）】**。
...
**无鉴权端点约定（V1 设备绑定模型，详见 §4.5.1）：**

| 端点 | 用途 | 鉴权方式 |
|------|------|---------|
| 本地操作端点（排盘 / 档案读写 / 本地规则分析） | 纯本地计算与存储 | **无需 Token**（由 `device_key` 本地识别） |
| `GET /api/v1/config/health` | 健康检查 | 无鉴权 |
| `POST /api/v1/users/register` | 云端账号注册 | 无鉴权（注册后获取 JWT） |
| `POST /api/v1/users/login` | 云端账号登录 | 无鉴权（登录后获取 JWT） |
| 云端算力端点（AI 深度分析 / 真太阳时校准等） | 需 LLM / 地理数据库 | **JWT**（`Authorization: Bearer <token>`） |

> ⚠️ 与早期 Web 式 Bearer 模型的区别：本地优先场景下，排盘、档案浏览等核心功能**不再要求 JWT**——它们由设备密钥本地鉴权，离线完全可用。JWT 仅用于云端账号与云端算力端点。

#### 4.5.1 设备绑定简化认证模型（V1）

> **来源：** ARC-15-P1-005（已确认方案 A — 设备绑定简化认证）。仅适用于纯桌面单机场景；Web / 移动端扩展见下方「V2 扩展条件」。

**方案描述：** APP 首次启动时自动生成唯一的设备密钥（`device_key`），存储在本地安全存储（Windows DPAPI / Credential Store）中。该设备密钥即本地操作的身份凭证，无需用户注册即可使用全部本地功能。JWT 仅在调用云端服务（LLM API、真太阳时校准）时才需要，用户在首次使用云端功能时才触发注册/登录流程。
...
| 维度 | 评估 |
|------|------|
| 零摩擦体验 | 打开即用，注册不再是强制步骤，满足"下载即用"的产品承诺 |
...
- 本地调用与云端调用使用不同的鉴权通道：本地 `device_key`、云端 `JWT`
...
**曾评估的替代方案（未采纳，保留参考）：**
- 方案 B：传统 JWT 认证（始终在线）——与 ADR-014「本地优先」核心原则严重冲突，离线不可用。
- 方案 C：Hybrid 本地优先认证（设备密钥 + JWT 无感刷新）——V1 用户量小、场景简单，实现成本不必要，建议 V2/V3 评估升级。
